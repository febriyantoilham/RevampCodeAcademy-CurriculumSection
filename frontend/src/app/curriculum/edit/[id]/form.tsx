...
import { CreateProgEntityIdReq, CreateProgramReq, DeleteProgramReq, EditProgramReq, ResetProgram } from "@/redux-saga/action/curriculum/programEntityAction";
...
export default function Form(props: any) {
    ...
    const [alertInfo, setAlertInfo] = useState({ showAlert: false, alertText: '', alertType: '' }); // Alert UI Config
    const program = props.program; // Program Data
    const category = props.option.category; // Category Data
    const instructor = props.option.instructor; // Instructor Data
    const [imageExists, setImageExists] = useState(true); // Check image
    const [onUpload, setOnUpload] = useState(false); // Check On Uploads
    const sectionList = useSelector((state: any) => state.sectionState.sectionList); // Section List State (sectionsReducer)
                ...
                <div id="form" className="xl:col-span-3">
                    <div id="program_entity">
                        <div className="grid xl:grid-cols-6 gap-x-2">
                            <div className="xl:col-span-1 flex flex-col mb-3">
                                ...
                                <input type="text" id="progEntityId" placeholder="Program ID" className="input input-bordered w-full capitalize" defaultValue={formik.values.progEntityId} disabled/>
                            </div>
                    ...
                    <div id="sections">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <CreateSectionPage progEntityId={progEntityId} setRefresh={props.setRefresh}/>
                        </div>
                        { program.length !== 0 && 
                            <div className="mt-3">
                                { program.sections.length !== 0 && sectionList ? (
                                    sectionList.map((section: any, index: any)=>(
                                        <div className="card card-compact w-full bg-base-200 mb-5" key={index}>
                                            <div className="card-body">
                                                <div className="flex justify-between">
                                                    <div className="text-xl font-medium">{section.sectTitle}</div>
                                                    <div className="flex">
                                                        <EditSectionPage section={section} progEntityId={progEntityId} setRefresh={props.setRefresh}/>
                                                    </div>
                                                </div>
