import { DeleteSectionDetailRequest } from "@/redux-saga/action/curriculum/sectionDetailAction";
...
export default function ViewSectionDetailPage(props: any) {
    ...
    return (
        <>
            {editView ? (<>
                <EditSectionDetailPage section={section} setView = {handleChange} setRefresh={props.setRefresh}/>
            </>)
            :(<>    
                <>
                                ...
                                section.sectionDetailMaterials[0].sedmFiletype === 'image' ? (<>
                                    <div className="avatar mb-3">
                                        <div className="w-24 m-auto">
                                            {section?.sectionDetailMaterials[0].sedmFilename === null || section?.sectionDetailMaterials[0].sedmFilename === '' ? (<Image src="/photo-def.png" alt={""} width={500} height={500}/>) : imageExists ? (<Image src={`${config.domain}/program_entity/subsection/get/file/${section?.sectionDetailMaterials[0].sedmFilename}`} alt={"dss"} width={500} height={500} onError={handleImageError}/>) : (<Image src="/photo-def.png" alt={""} width={500} height={500}/>)}
                                        </div>
                                    </div>
